...
const render = () => {
  ...
  } else document.querySelector('.current').textContent = '';
};
...
const calculate = arr => {
  ...
  const add = op => temp[op - 1] + temp[op + 1],
    ...
    mergeValues = (op, func) => {
      temp =
        op === temp.length - 2
          ? temp.slice(0, op - 1).concat([func(op)])
          : temp
              .slice(0, op - 1)
              .concat([func(op)], temp.slice(op + 2, temp.length));
    };

  temp.forEach((e, i) => {
    if (/[0-9]/.test(e)) temp[i] = Number(temp[i]);
  });

  for (let i = 0; i < temp.length; i++) {
    if (temp[i] === '(') {
      pLevel++;
      if (pLevel === 1) lList.push(i);
    } else if (temp[i] === ')') {
      pLevel--;
      if (pLevel === 0) rList.push(i);
    }
  }
  ...
  for (let i = 0; i < lList.length; i++) {
    ...
    if (temp.includes(false)) return false;
...
document.querySelectorAll('.num').forEach(e => {
  e.addEventListener('click', event => {
    ...
    else mem[mem.length - 1] = event.target.textContent;
    render();
  });
});
...
document.querySelectorAll('.op').forEach(e => {
  e.addEventListener('click', event => {
    if (calculated) {
      ...
      if (mem[mem.length - 1] === '.') mem[mem.length - 1] = '0.';
...
document.querySelector('.clear-entry').addEventListener('click', () => {
  ...
  if (mem.length > 1) mem.pop(mem.length - 1);
  else mem = [];
  render();
});

document.querySelector('.parL').addEventListener('click', () => {
  if (calculated) {
    renderLast(`Ans = ${ans} `);
    calculated = false;
  }
...
document.querySelector('.parR').addEventListener('click', () => {
  if (
    mem[mem.length - 1] !== '(' &&
